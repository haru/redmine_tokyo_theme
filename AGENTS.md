...
This is the canonical guidance document for this repository. `CLAUDE.md` and `.github/copilot-instructions.md` point here rather than duplicating this content — update this file when project conventions change.
...
- Variables: camelCase (`var itemCount`)
...
- **Path Portability**: `@import url(../../../stylesheets/application.css)` in `application.css` must not change; theme must work regardless of Redmine's deployment path (subdir installations, etc.)
